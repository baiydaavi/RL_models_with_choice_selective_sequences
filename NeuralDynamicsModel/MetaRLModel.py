"""This module defines the neural dynamics model."""
import os
import tqdm
import numpy as np
import tensorflow as tf

from training_utils import get_expected_return, compute_loss_policy, compute_loss_value
...
class MetaRLModel:
    ...
    def train(self, load_model=None):
        ...
        Args:
            load_model (str, optional): Path of the trained model. Defaults to None.
        """
        ...
        with tqdm.trange(self.max_episodes) as t:
            for episode in t:
                ...
                initial_obs_state, initial_reward, _, _ = self.env.step(initial_action)
                ...
                initial_critic_obs = tf.concat([choice_act, [initial_reward]], 0)
                ...
                returns = get_expected_return(
                    tf.concat([rewards, [values[-1]]], 0), tf.concat([gammas, [1.0]], 0)
                )[:-1]
                ...
                t.set_postfix(episode_reward=episode_reward, episode_loss=episode_loss)
    ...
    def train_episode(
        ...
        Args:
            in_critic_obs (tf.Tensor): Input to the critic.
            in_actor_obs (tf.Tensor): Input to the actor.
            initial_state (tf.Tensor): Initial state of the network.

        Returns:
            Training data collected over the entire episode.
        """

        critic_obs_seq = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True)
        ...
        for step_num in tf.range(self.max_steps_per_episode):
            ...
            (
                ...
                actor_temp_h,
                actor_temp_c,
            ) = self.model(critic_obs, actor_obs, state_critic, state_actor)
            ...
            obs_state, reward, _, current_episode_time = self.env.step(action)
            ...
            if self.env.choice == 1:
                choice_act = tf.convert_to_tensor(
                    self.left_choice_activity[
                        self.env.rand_activity_trial_num, :, obs_state
                    ],
                    dtype=tf.float32,
                )
    ...
    def train_step(
        ...
        """Updates model parameters and returns total loss.

        Args:
            critic_obs (tf.Tensor): List of critic inputs.
            actor_obs (tf.Tensor): List of actor inputs.
            actions (tf.Tensor): List of actions.
        ...
        with tf.GradientTape() as tape:
            logits, values, _, _, _, _, _ = self.model(
                critic_obs, actor_obs, initial_state, initial_state
            )
            ...
            for item in tf.range(len(actions)):
                action_probs = action_probs.write(item, probs[item, actions[item]])
            ...
            loss = compute_loss_policy(
                action_probs, probs, advantages
            ) + compute_loss_value(values, returns)
    ...
    def test(
        self,
        save_destination_folder,
        run_id=0,
        load_model=None,
        fraction_stimulated=0.7,
        stimulation_level=0.15,
    ):
        """Runs the test for a given number of episodes and saves results.

        Args:
            save_destination_folder (str): Location to save the test results.
            run_id (int, optional): Current run instance. Defaults to 0.
            load_model (str, optional): Location of the trained model. Defaults to None.
            fraction_stimulated (float, optional): Fraction of neurons to stimulate if
            the trial is stimulated. Defaults to 0.7.
            stimulation_level (float, optional): Level of stimulation applied to the
            neurons if the trial is stimulated. Defaults to 0.15.
        """
    ...
    def test_episode(
        ...
        Args:
            in_critic_obs (tf.Tensor): Initial input to the critic.
            in_actor_obs (tf.Tensor): Initial input to the actor.
            initial_state (tf.Tensor): Initial state of the network.
            fraction_stimulated (float, optional): Fraction of neurons to stimulate if
            the trial is stimulated. Defaults to 0.7.
            stimulation_level (float, optional): Level of stimulation applied to the
        ...
        Returns:
            Testing data collected over the entire episode.
        """
        ...
        for _ in tqdm.tqdm(range(self.max_steps_per_episode)):
            ...
            # Apply action to the environment to get next state and reward
            obs_state, reward, is_rewarded, trial_count_in_block = self.env.step(action)
            ...
            if self.env.choice == 1:
                if self.env.is_stimulated == 1:
                    ...
                    stimulated_neurons = np.random.choice(
                        num_neurons,
                        int(fraction_stimulated * num_neurons),
                        replace=False,
                    )
                    choice_act[stimulated_neurons] = stimulation_level
                    choice_act = tf.convert_to_tensor(choice_act, dtype=tf.float32)
                else:
                    choice_act = tf.convert_to_tensor(
                        self.left_choice_activity[
                            self.env.rand_activity_trial_num, :, obs_state
    ...
    def load_weights(self, load_model=None):
        ...
        Returns:
            Dictionary of the model weights.
        """
        if load_model:
            self.model.load_weights(load_model)
        ...
        return {
            "ActorOutput": self.model.actor.get_weights(),
